...
use crate::definitions::{parse_definitions, SystemCall};
...
impl<'a> Source<'a> {
    ...
    async fn generate(&self, dir: &Path, definitions: &HashMap<String, SystemCall>) -> Result<(&str, PathBuf)> {
        let table = self
            .fetch_table()
            .await
        ...
        write!(file, "{}", SyscallFile(&table, definitions))?;

        Ok((self.arch, dir))
    }
}

struct SyscallFile<'a>(&'a [TableEntry], &'a HashMap<String, SystemCall>);
...
impl<'a> fmt::Display for SyscallFile<'a> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        writeln!(f, "use lazy_static::lazy_static;")?;
        writeln!(f, "use std::collections::HashMap;")?;
        writeln!(f, "use super::super::Argument;")?;
        writeln!(f, "")?;
        ...
        writeln!(f, "    pub static ref DEFINITIONS: HashMap<Sysno, Vec<Argument>> = HashMap::from([")?;

        for entry in self.0 {
            let definition = match self.1.get(&entry.name) {
                Some(definition) => definition,
                _ => continue,
            };

            writeln!(f, "        (")?;
            writeln!(f, "            Sysno::{},", entry.name)?;
            writeln!(f, "            vec![")?;

            for argument in &definition.arguments {
                writeln!(f, "                Argument {{")?;
                writeln!(f, "                    ty: \"{}\".to_string(),", argument.ty)?;
                writeln!(f, "                    name: \"{}\".to_string(),", argument.name)?;
                writeln!(f, "                }},")?;
            }
...
async fn main() -> Result<()> {
    ...
    let definitions = parse_definitions("/usr/src/linux")?;

    for source in SOURCES.iter() {
        let (arch, path) = source.generate(&base_dir, &definitions).await?;
